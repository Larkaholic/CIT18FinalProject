<x-app-layout>
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 -mt-4">
        <h1 class="text-2xl font-semibold mb-4 text-white">Genres</h1>
        ...
        {{-- Movies by Genre --}}
        @if (isset($genre))
            <h2 class="text-xl font-semibold mb-4 text-white">Movies in {{ $genre }} Genre</h2>
            @if (isset($movies) && $movies->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($movies as $movie)
                        <div class="border rounded-md p-4 border-gray-700 bg-gray-800">
            ...
            @else
                <p class="text-white">No movies found in {{ $genre }} genre.</p>
            @endif
        @else
            <p class="text-white">Select a genre to view movies.</p>
        @endif
